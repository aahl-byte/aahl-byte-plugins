...
- **Conversational phases** (vision, design, review) — need user interaction. Invoke via the Skill tool in the current session. After the skill completes, launch a **fresh terminal** for the next phase using `tsx ${CLAUDE_PLUGIN_ROOT}/scripts/dk/launch.ts {initiative-name}` via the Bash tool. This gives the next session a clean context window.
...
```
tsx ${CLAUDE_PLUGIN_ROOT}/scripts/dk/launch.ts {name} [area-slug]
```
...
```
tsx ${CLAUDE_PLUGIN_ROOT}/scripts/dk/launch.ts {name}
```
...
|--------------|------------|------|
| vision (exploring → diving) | design | `tsx ${CLAUDE_PLUGIN_ROOT}/scripts/dk/launch.ts` |
| design (dive complete, more remaining) | design | `tsx ${CLAUDE_PLUGIN_ROOT}/scripts/dk/launch.ts` |
| design (all dives complete) | vision (consolidation) | `tsx ${CLAUDE_PLUGIN_ROOT}/scripts/dk/launch.ts` |
| vision (consolidation → ready) | plan | invoke directly |
| plan (complete) | review | `tsx ${CLAUDE_PLUGIN_ROOT}/scripts/dk/launch.ts` |
